const Questions = () => {
    const questions_text = [
        ...
        '군대 면제인데, 그 이유는 무엇인가요?',
        '업무에 도움이 될 만한 자격증이 없는데, 업무하는데 지장이 있지 않을까요?',
        ...
        '자신의 옆에 두 명보다 어떤 점이 뛰어나 뽑혀야만 된다고 생각합니까?',
        '몇 번째 면접입니까? 그 동안 왜 떨어졌다고 생각합니까?'
        ]
    return (
        <div className="Questions">
            {
                questions_text.map((questions_text, index) => {
                    return (
                        <p className="text">{questions_text}</p>
                    )
                })
            }
        </div>
